...
class FishingBot():
    ...
    def __init__(self, settings_helper, *args, **kwargs):
        ...
        self.game_window_name = self.settings_helper.settings['client'].get('game_window_name')
        self.game_window_class = self.settings_helper.settings['client'].get('game_window_class')
    ...
    def find_template(self,
        ...
        for scale in np.linspace(scale_min, scale_max, scale_steps)[
                     ::-1]:  # start_scale, max_scale, total increments between start-max
    ...
    def count_loot(self, loot_box):
        ...
        while get_duration(then=start_time, now=datetime.now(),
                           interval='seconds') < 3 and self.break_helper.time_to_break == False:
            with mss.mss() as sct:
                ...
                for fish_name, fish_data in self.fish_map.items():
                    ...
                    logger.debug(
                        f'Template: {fish_name} | Confidence: {confidence} | Location: {location} | Scale: {scale}')
    ...
    def catch_fish(self, first_x, first_y):
        ...
        while get_duration(then=start_time, now=datetime.now(), interval='seconds') < self.settings_helper.settings[
            'fishing'].getint('timeout_threshold') and self.break_helper.time_to_break == False:
            with mss.mss() as sct:
                # Take screenshot of the bobber_box area
                ...
                confidence, x, y = AiHelper.find_location(screenshot)
                ...
                if (confidence >= 0.30):
                    ...
                    logger.debug(f'Checking if {y} - {average_y_value} >= {self.DIP_THRESHOLD}')
                    if (abs(y - average_y_value) >= self.DIP_THRESHOLD):
                        # Change in y position > threshold so we're going to click the bobber / catch the fish.
                        self.input_helper.move_mouse(first_x, first_y)
                        self.input_helper.click_mouse("right")
                        return True
    ...
    def run_fish_loop(self):
        ...
        while not self.break_helper.time_to_break:
            ...
            with mss.mss() as sct:
                ...
                confidence, first_x, first_y = AiHelper.find_location(screenshot)

                logger.debug(f'Potential Bobber - Confidence: {confidence} | Location: {first_x, first_y}')
                ...
                if confidence >= self.settings_helper.settings['fishing'].getfloat('min_confidence'):
                    logger.success(f"Bobber Found | Confidence: {confidence} ")
                    ...
                    # Wait for catch
                    if (self.catch_fish(first_x, first_y)):
                        # Identify loot and add it to counter for progress report
                        self.fish_caught += 1


                        # loot_screen_coords = self.translate_coords(loot_loc,
    ...
    def send_stats(self, game_screenshot):
        ...
        gold_earned = round(
            sum([self.fish_map[fish]['loot_count'] * self.tsm.fish_map[fish]['price'] for fish in self.tsm.fish_map]),
            2)
            ...
            async def send_discord_progress_report():
                async with aiohttp.ClientSession() as session:
                    ...
                    await webhook.send(embed=embed, file=file,
                                       username=self.settings_helper.settings['user'].get('nickname'))
    ...
    def set_game_window_data(self):
        ...
        # GxWindowClass, Window Text: 魔兽世界
        self.game_window_handle = FindWindow(self.game_window_class, self.game_window_name)
        # self.game_window_handle = FindWindow('GxWindowClass', '魔兽世界')
